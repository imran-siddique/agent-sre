...
class TestRunbookExecutor:
    def test_runs_steps_in_order(self) -> None:
        ...
        def step_a(inc: Incident) -> str:
            order.append("a")
            return "a done"

        def step_b(inc: Incident) -> str:
            order.append("b")
            return "b done"
        ...
        rb = Runbook(
            ...
            name="Order Test",
            steps=[
                RunbookStep(name="step-a", action=step_a),
                RunbookStep(name="step-b", action=step_b),
            ],
        )
        executor = RunbookExecutor()
        result = executor.execute(rb, _make_incident())
    ...
    def test_string_actions(self) -> None:
        ...
        result = executor.execute(rb, _make_incident())
        assert result.status == ExecutionStatus.COMPLETED
        assert "[command]" in result.step_results[0].output
    ...
    def test_step_failure_marks_failed(self) -> None:
        ...
        assert "step failed" in result.step_results[-1].error

    def test_rollback_on_failure(self) -> None:
        rolled_back: list[str] = []

        def rb_action(inc: Incident) -> str:
        ...
        rb = Runbook(
            ...
            name="Rollback Test",
            steps=[
                RunbookStep(name="ok", action=_ok_action, rollback_action=rb_action),
                RunbookStep(name="boom", action=_fail_action),
            ],
        )
        executor = RunbookExecutor()
    ...
    def test_approval_callback_approved(self) -> None:
        ...
        executor = RunbookExecutor()
        result = executor.execute(rb, _make_incident(), approve_callback=lambda s, i: True)
        assert result.status == ExecutionStatus.COMPLETED
    ...
    def test_approval_callback_denied(self) -> None:
        ...
        executor = RunbookExecutor()
        result = executor.execute(rb, _make_incident(), approve_callback=lambda s, i: False)
        assert result.status == ExecutionStatus.FAILED
    ...
    def test_no_callback_pauses_with_waiting(self) -> None:
        ...
        assert result.step_results[0].status == StepStatus.WAITING_APPROVAL
