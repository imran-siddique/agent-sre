...
class TestParetoFrontier:
    def test_no_dominated_models(
        ...
        for i, a in enumerate(frontier):
            for j, b in enumerate(frontier):
                if i == j:
                    continue
                # No model should dominate another on the frontier
                assert not (
                    a.estimated_cost <= b.estimated_cost
                    and a.estimated_quality >= b.estimated_quality
                    and (
                        a.estimated_cost < b.estimated_cost
                        or a.estimated_quality > b.estimated_quality
                    )
                )
    ...
    def test_frontier_sorted_by_cost(
        ...
        frontier = optimizer.pareto_frontier(summarization_task)
        costs = [e.estimated_cost for e in frontier]
        assert costs == sorted(costs)
    ...
    def test_frontier_includes_extremes(
        ...
        frontier = optimizer.pareto_frontier(summarization_task)
        names = [e.model_name for e in frontier]
        # Cheapest and highest-quality should be on frontier
...
class TestSimulation:
    def test_simulation_correct_totals(
        ...
        result = optimizer.simulate(summarization_task, "gpt-4o-mini", volume=1000)
        assert result["volume"] == 1000
        assert result["cost_per_invocation"] == pytest.approx(0.0006, abs=1e-6)
        assert result["total_cost"] == pytest.approx(0.6, abs=1e-4)
        assert result["total_input_tokens"] == 2_000_000
