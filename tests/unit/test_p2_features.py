...
class TestDistributedReplay:
    ...
    def test_add_and_replay(self):
        ...
        assert result.state == MeshReplayState.COMPLETED
    ...
    def test_discover_links(self):
        ...
        assert len(links) == 1
        assert links[0].from_agent == "agent-a"
...
class TestChaosLibrary:
    ...
    def test_get_template(self):
        lib = ChaosLibrary()
        t = lib.get("tool-timeout")
        assert t is not None
        assert t.category == "tool"

    def test_instantiate(self):
        lib = ChaosLibrary()
        exp = lib.instantiate("tool-timeout", "agent-1")
...
class TestPostmortem:
    ...
    def test_generate_postmortem(self):
        ...
        assert pm.severity == IncidentSeverity.P2
        assert len(pm.timeline) >= 3  # detected + signal + action + resolved

    def test_postmortem_summary(self):
        gen = PostmortemGenerator()
        pm = gen.generate(self._make_incident())
        assert "agent-1" in pm.summary
