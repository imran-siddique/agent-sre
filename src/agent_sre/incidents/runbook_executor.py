"""Runbook executor — sequential step execution with approval gates and rollback."""
...
class RunbookExecutor:
    ...
    def execute(
        ...
        Args:
            ...
            incident: The incident triggering this runbook.
            approve_callback: Optional callback for steps requiring approval.
                Receives (step, incident) and returns True to approve, False to reject.
                If None and a step requires approval, the execution pauses with
        ...
        for step in runbook.steps:
            ...
            if step.requires_approval:
                if approve_callback is None:
                    ...
                    self._emit_event(
                        "waiting_approval", step=step, runbook=runbook, incident=incident
                    )
                    self._executions.append(execution)
                    return execution

                self._emit_event(
                    "approval_requested", step=step, runbook=runbook, incident=incident
                )
                approved = approve_callback(step, incident)
                if not approved:
                    result.status = StepStatus.SKIPPED
                    result.output = "Approval denied"
                    execution.step_results.append(result)
                    self._emit_event(
                        "approval_denied", step=step, runbook=runbook, incident=incident
                    )
            ...
            self._emit_event("step_started", step=step, runbook=runbook, incident=incident)
            ...
            try:
                ...
                self._emit_event(
                    "step_completed", step=step, runbook=runbook, incident=incident, output=output
                )
                ...
                execution.step_results.append(result)
                self._emit_event(
                    "step_failed", step=step, runbook=runbook, incident=incident, error=str(exc)
                )
