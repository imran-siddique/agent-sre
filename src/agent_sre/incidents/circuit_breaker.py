...
class CircuitBreaker:
    ...
    - OPEN: Agent isolated. Reject all calls. Wait for timeout.
    - HALF_OPEN: Allow limited test calls. If they succeed, close. If they fail, reopen.
    """
    ...
    def state(self) -> CircuitState:
        """Current state, auto-transitioning from OPEN to HALF_OPEN on timeout."""
        if self._state == CircuitState.OPEN and self._opened_at is not None:
            if time.time() - self._opened_at >= self.config.timeout_seconds:
                self._transition(CircuitState.HALF_OPEN, "timeout expired")
        return self._state
    ...
    def record_failure(self, error: str = "") -> None:
        ...
        if state == CircuitState.HALF_OPEN:
            self._half_open_calls += 1
            self._transition(CircuitState.OPEN, f"failure during recovery: {error}")
        elif state == CircuitState.CLOSED:
