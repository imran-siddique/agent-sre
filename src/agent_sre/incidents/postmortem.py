...
class PostmortemGenerator:
    ...
    def generate(self, incident: Incident) -> Postmortem:
        ...
        # Build summary
        pm.summary = self._build_summary(incident)
        pm.impact = self._build_impact(incident)
        pm.detection = self._build_detection(incident)
        pm.response = self._build_response(incident)
        pm.root_cause = self._analyze_root_cause(incident)

        # Build timeline from incident lifecycle
        pm.add_timeline_entry("Incident detected", "agent-sre", incident.title, incident.detected_at)
        for signal in incident.signals:
            pm.add_timeline_entry(
        ...
        if incident.resolved_at:
            pm.add_timeline_entry("Incident resolved", "agent-sre", "", incident.resolved_at)

        # Generate action items
        pm.action_items = self._suggest_actions(incident)
        pm.lessons_learned = self._suggest_lessons(incident)
        pm.contributing_factors = self._identify_factors(incident)
