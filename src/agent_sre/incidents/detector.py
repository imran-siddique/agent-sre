"""Incident detection — SLO breach alerting and response."""
...
class IncidentDetector:
    ...
    Correlates multiple weak signals into strong alerts,
    deduplicates, and routes to appropriate severity.
    """
    ...
    def ingest_signal(self, signal: Signal) -> Incident | None:
        ...
        Returns an Incident if one is created, None otherwise.
        """
    ...
    def _create_correlated_incident(self, signals: list[Signal]) -> Incident:
        ...
        incident = Incident(
            title=f"Correlated signals: {', '.join(types)} from {', '.join(sources)}",
            severity=IncidentSeverity.P2,
            signals=signals,
            agent_id=signals[0].source,
        )
        self._incidents.append(incident)
        return incident
    ...
    def _find_correlated(self, signal: Signal) -> list[Signal]:
        """Find signals correlated with the given signal (same source, recent)."""
        cutoff = time.time() - self.correlation_window
        return [
            s for s in self._pending_signals
            if s.source == signal.source and s.timestamp >= cutoff
        ]
