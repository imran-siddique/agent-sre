...
class ReplayEngine:
    ...
    def diff(self, trace_a: Trace, trace_b: Trace) -> list[TraceDiff]:
        ...
        Useful for comparing a production execution with a replay,
        or two versions of the same agent on the same input.
        """
        ...
        for name in sorted(all_names):
            ...
            # Compare cost
            if sa.cost_usd != sb.cost_usd:
                diffs.append(TraceDiff(
                    diff_type=DiffType.COST_CHANGE,
                    span_name=name,
                    original=sa.cost_usd,
                    replayed=sb.cost_usd,
                    description=f"Cost changed for '{name}': ${sa.cost_usd:.4f} → ${sb.cost_usd:.4f}",
                ))

        # Check tool call sequence
        tools_a = [s.name for s in trace_a.spans if s.kind == SpanKind.TOOL_CALL]
        tools_b = [s.name for s in trace_b.spans if s.kind == SpanKind.TOOL_CALL]
        if tools_a != tools_b:
            diffs.append(TraceDiff(
                diff_type=DiffType.TOOL_SEQUENCE_DIFF,
                span_name="__sequence__",
                original=tools_a,
                replayed=tools_b,
    ...
    def what_if(self, trace: Trace, overrides: dict[str, dict[str, Any]]) -> ReplayResult:
        """Counterfactual analysis: replay with modified inputs/outputs.

        Args:
            trace: Original trace.
            overrides: Map of span name → modified output_data.

        Returns:
            ReplayResult showing divergences from the override.
        """
